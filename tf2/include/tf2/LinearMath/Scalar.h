...
/// The tf2Scalar type abstracts floating point numbers, to easily switch between double and single
...
/// tf2Select avoids branches, which makes performance much better for consoles like Playstation 3
/// and XBox 360 Thanks Phil Knight. See also
...
TF2SIMD_FORCE_INLINE int tf2Select(unsigned condition, int valueIfConditionNonZero,
                                   int valueIfConditionZero) {
  unsigned testNz = (unsigned)(((int)condition | -(int)condition) >> 31);
  unsigned testEqz = ~testNz;
  return static_cast<int>((valueIfConditionNonZero & testNz) | (valueIfConditionZero & testEqz));
}
TF2SIMD_FORCE_INLINE float tf2Select(unsigned condition, float valueIfConditionNonZero,
...
/// for you. so instead of returning a float/double, we return integer/long long integer
