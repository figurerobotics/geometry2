 ...
 * It has an un-used w component to suit 16-byte alignment when tf2::Vector3 is stored in
 * containers. This extra component can be used by derived classes (Quaternion?) or by user Ideally,
 * this class should be replaced by a platform optimized TF2SIMD version that keeps the data in
 * registers
 */
ATTRIBUTE_ALIGNED16(class) Vector3 {
 ...
 public:
  ...
  /// operator tf2Scalar*() replaces operator[], using implicit conversion. We added operator != and
  /// operator == to avoid pointer comparisons.
...
class tf2Vector4 : public Vector3 {
 public:
  ...
  TF2SIMD_FORCE_INLINE tf2Vector4(const tf2Scalar& x, const tf2Scalar& y, const tf2Scalar& z,
                                  const tf2Scalar& w)
      : Vector3(x, y, z) {
    m_floats[3] = w;
  }
...
/// tf2SwapVector3Endian swaps vector endianness, useful for network and cross-platform
/// serialization
...
/// tf2UnSwapVector3Endian swaps vector endianness, useful for network and cross-platform
/// serialization
